...
System.out.println("Do you want to add new school programm?");
	...
	public static void EnterLesson() throws Exception {
		...
		int rows = -1;
		int numOfRows = sh.getLastRowNum();
boolean EnterMatematics=false;
Scanner scan=new Scanner(System.in);
System.out.println("Do you want to add new account!!!");
...
File file = new File(System.getProperty("user.dir") + "\\Mat\\Starter.vbs");
	...
	//	driver.findElement(By.cssSelector("body")).click();
		driver.get("https://meet.google.com/?authuser=1");
		if(dec.equals("yes")) 
		{
			Change(driver,name,pass);
		...
		while (true) {
			...
			try {
				...
				}else if(lesson.equals("mat")) 
				{
					EnterMat(driver);
					EnterMatematics=true;
...
catch(Exception ex) 
{
	...
	public static String getHtmlLink(WebDriver driver) throws Exception {
		...
		String value = driver.findElement(By.xpath("//a[contains(text(),'meet')]")).getText();
...
public static void EnterMat(WebDriver driver) 
{
	
	driver.findElement(By.cssSelector("body")).click();
	
}
public static void Change(WebDriver driver,String name,String pass)  
{
	
	driver.findElement(By.cssSelector("body")).click();
	driver.get("https://accounts.google.com/signin/v2/identifier?continue=https%3A%2F%2Fmeet.google.com%2Flanding%3Fauthuser%3D1&ec=GAlAmwM&flowName=GlifWebSignIn&flowEntry=AddSession");
	driver.manage().timeouts().implicitlyWait(60, TimeUnit.SECONDS);
	driver.findElement(By.xpath("//*[@id=\"identifierId\"]")).sendKeys(name);
